...
export default function StaffImportPage() {
  ...
  async function createJob(e: React.FormEvent) {
    ...
    const res = await fetch('/api/admin/imports/staff', { method: 'POST', body: fd })
  ...
  async function processJob() {
    ...
    const res = await fetch(`/api/admin/imports/staff/${jobId}/process`, { method: 'POST' })
    const text = await res.text()
    let data: any = {}
    try { data = JSON.parse(text) } catch {}
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-100 via-white to-amber-100">
      <div className="max-w-4xl mx-auto px-8 py-12">
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-sky-200">
          <h1 className="text-2xl font-bold text-sky-700 mb-2">Bulk Import: Staff (CSV)</h1>
          <p className="text-sm text-gray-600 mb-6">
            Upload a CSV to create an import job, then process it. Audit-logged.
          </p>
          ...
          <div className="bg-sky-50 border border-sky-200 rounded-xl p-4 mb-6 text-sm text-sky-800">
            ...
            <div className="font-mono text-xs">
              staff_number,surname,other_names,full_name,employment_type,role_title,employment_status
            </div>
          </div>
          ...
          {jobId && (
            <div className="mt-6 p-4 border rounded bg-gray-50">
              <div className="text-sm text-gray-700">Job ID created. You can now process it.</div>
              <button
                onClick={processJob}
